...
class UnityCodeGen:
    ...
    def Process(filepath, fields, table):
        ...
        # -----------------------table ConfigManager manager class-----------------------
        filecontent += "\n"
        tableclassmgrname = tablebasename + "ConfigManager"
        ...
        #  GetAllData函数
        filecontent += UnityCodeGen.Tab(1) + "\n"
        filecontent += UnityCodeGen.Tab(1) + "public List<{0}> GetAllData()\n".format(tableclassname)
        filecontent += UnityCodeGen.Tab(1) + "{\n"
        filecontent += UnityCodeGen.Tab(2) + "return mDict.Values.ToList();\n"
        filecontent += UnityCodeGen.Tab(1) + "}\n"
        #  GetData函数
        if keylen == 1:     # 有一个key值使用dict取值
            ...
            filecontent += UnityCodeGen.Tab(2) + "}\n"
            filecontent += UnityCodeGen.Tab(2) + "else\n"
            filecontent += UnityCodeGen.Tab(2) + "{\n"
            filecontent += UnityCodeGen.Tab(3) + "Debug.LogError(\"{0}表格配置错误！错误ID:\"+{1});\n".format(tableclassname,keyname.lower())
    ...
    def GenConfigMangerCode(files):
        ...
        for file in files:
            ...
            tablebasename = tablebasename.split(".")[0]
            filecontent += UnityCodeGen.Tab(2) + tablebasename + "ConfigManager.Instance.Deserialize(dynamicPacket);\n"
        filecontent += UnityCodeGen.Tab(1) + "}\n"

        # LoadCsv函数
        filecontent += UnityCodeGen.Tab(1) + "\n"
        filecontent += UnityCodeGen.Tab(1) + "public static void LoadConfig(string Configdatapath)\n"
        filecontent += UnityCodeGen.Tab(1) + "{\n"
        filecontent += UnityCodeGen.Tab(2) + "FileStream fileStream = new FileStream(Configdatapath, FileMode.Open, FileAccess.Read);\n"
